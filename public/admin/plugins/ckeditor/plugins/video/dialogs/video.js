...
( function() {
	var videoDialog = function( editor, dialogType ) {
			...
			var updatePreview = function( dialog ) {
					...
					// Read attributes and update videoPreview;
			...
			var switchLockRatio = function( dialog, value ) {
					...
					// Check video ratio and original video ratio, but respecting user's preference.
					if ( value == 'check' ) {
			...
			var onImgLoadEvent = function() {
					...
					var original = this.originalElement,
						loader = CKEDITOR.document.getById( videoPreviewLoaderId );
					...
					// New video -> new dimensions
					if ( !this.dontResetSize ) {
						resetSize( this, editor.config.video_prefillDimensions === false );
					}
			...
			var onImgLoadErrorEvent = function() {
					...
					// Set Error video.
					var novideo = CKEDITOR.getUrl( CKEDITOR.plugins.get( 'video' ).path + 'videos/novideo.png' );

					if ( this.preview )
						this.preview.setAttribute( 'src', novideo );
			...
			var numbering = function( id ) {
				...
				videoPreviewLoaderId = numbering( 'ImagePreviewLoader' ),
				previewLinkId = numbering( 'previewLink' ),
				previewImageId = numbering( 'previewImage' );

			return {
				title: editor.lang.image[ dialogType == 'video' ? 'title' : 'titleButton' ],
				minWidth: 420,
				minHeight: 360,
				onShow: function() {
					...
					// Copy of the video
					this.originalElement = editor.document.createElement( 'video' );
					this.originalElement.setAttribute( 'controls', 'true' );
					...
					if ( link ) {
						...
						if ( linkChildren.count() == 1 ) {
							...
							if ( childTag.type == CKEDITOR.NODE_ELEMENT ) {
								if ( childTag.is( 'video' ) || childTag.is( 'input' ) ) {
									this.videoElement = linkChildren.getItem( 0 );
									if ( this.videoElement.is( 'video' ) )
										this.videoEditMode = 'video';
									else if ( this.videoElement.is( 'input' ) )
										this.videoEditMode = 'input';
								}
							}
						}
						// Fill out all fields.
					...
					// Edit given video element instead the one from selection.
					if ( this.customImageElement ) {
						this.videoEditMode = 'video';
						this.videoElement = this.customImageElement;
						delete this.customImageElement;
					}
					else if ( element && element.getName() == 'video' && !element.data( 'cke-realelement' ) ||
						element && element.getName() == 'input' && element.getAttribute( 'type' ) == 'video' ) {
						this.videoEditMode = element.getName();
						this.videoElement = element;
					}

					if ( this.videoEditMode ) {
						// Use the original element as a buffer from  since we don't want
						// temporary changes to be committed, e.g. if the dialog is canceled.
						this.cleanImageElement = this.videoElement;
						this.videoElement = this.cleanImageElement.clone( true, true );

						// Fill out all fields.
						this.setupContent( IMAGE, this.videoElement );
					}
				...
				onOk: function() {
					// Edit existing Image.
					if ( this.videoEditMode ) {
						var imgTagName = this.videoEditMode;

						// Image dialog and Input element.
						if ( dialogType == 'video' && imgTagName == 'input' && confirm( editor.lang.image.button2Img ) ) { // jshint ignore:line
							// Replace INPUT-> IMG
							imgTagName = 'video';
							this.videoElement = editor.document.createElement( 'video' );
							this.videoElement.setAttribute( 'controls', 'true' );
							editor.insertElement( this.videoElement );
						}
						// ImageButton dialog and Image element.
						else if ( dialogType != 'video' && imgTagName == 'video' && confirm( editor.lang.image.img2Button ) ) { // jshint ignore:line
							// Replace IMG -> INPUT
							imgTagName = 'input';
							this.videoElement = editor.document.createElement( 'input' );
							this.videoElement.setAttributes( {
								type: 'video',
								controls: 'true'
							} );
							editor.insertElement( this.videoElement );
							...
							this.videoElement = this.cleanImageElement;
					...
					// Create a new video.
						...
						if ( dialogType == 'video' )
							...
							this.videoElement = editor.document.createElement( 'input' );
							this.videoElement.setAttribute( 'type', 'video' );
						}
						this.videoElement.setAttribute( 'controls', 'true' );
					}
					...
					// Set attributes.
					this.commitContent( IMAGE, this.videoElement );
					this.commitContent( LINK, this.linkElement );

					// Remove empty style attribute.
					if ( !this.videoElement.getAttribute( 'style' ) )
					...
					if ( !this.videoEditMode ) {
						if ( this.addLink ) {
							if ( !this.linkEditMode ) {
								...
								this.linkElement.append( this.videoElement, false );
							} else {
								...
								if ( this.linkElement.equals( editor.getSelection().getSelectedElement() ) ) {
									...
									// Only inside of the link is selected, so replace it with video. (<a>[foo]</a>, <a>[f]oo</a>)
									editor.insertElement( this.videoElement );
								}
							}
						} else {
					...
					// Image already exists.
						...
						if ( !this.linkEditMode && this.addLink ) {
							editor.insertElement( this.linkElement );
							this.videoElement.appendTo( this.linkElement );
						}
							...
							children: [ {
								...
								onChange: function() {
									...
									if ( newUrl.length > 0 ) {
										...
										// Show loader.
										var loader = CKEDITOR.document.getById( videoPreviewLoaderId );
										if ( loader )
						...
						commit: function( type, element ) {
							if ( type == IMAGE ) {
								if ( this.getValue() || this.isChanged() )
									element.setAttribute( 'controls', this.getValue() );
						...
						},
						{
							...
							children: [ {
								...
								html: '<div>' + CKEDITOR.tools.htmlEncode( editor.lang.common.preview ) + '<br>' +
									'<div id="' + videoPreviewLoaderId + '" class="ImagePreviewLoader" style="display:none"><div class="loading">&nbsp;</div></div>' +
									'<div class="ImagePreviewBox"><table><tr><td>' +
										'<video id="' + previewImageId + '" controls="true" ></video>' +
									// jscs:disable maximumLineLength
										( editor.config.video_previewText || 'Lorem ipsum dolor sit amet, consectetuer adipiscing elit. ' +
						...
						commit: function( type, element ) {
							if ( type == LINK ) {
								if ( this.getValue() || this.isChanged() ) {
									...
									if ( this.getValue() || !editor.config.video_removeLinkByEmptyURL )
	...
	CKEDITOR.dialog.add( 'videobutton', function( editor ) {
		return videoDialog( editor, 'videobutton' );
	} );
} )();
